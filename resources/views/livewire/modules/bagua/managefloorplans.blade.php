...
new class extends Component {
    ...
    {
        return [
            'plans' => $this->project->floorPlans()->with(['media', 'baguaNotes'])->orderBy('sort_order')->get(),
        ];
    }
};
?>
                    ...
                    @if($plan->hasMedia('floor_plans'))
                        ...
                        @if($bounds)
                            <svg class="absolute inset-0 w-full h-full pointer-events-none z-[5]"
                                ...
                                {{-- Bagua Labels (Asian names or German aspirations) --}}
                                @foreach($plan->baguaNotes as $note)
                                    @php
                                        ...
                                        if (!$data && $note->content) {
                                            $data = json_decode(json_decode($note->content, true), true) ?? [];
                                        }

                                        $showAspirations = auth()->user()->settings['show_bagua_aspirations'] ?? false;
                                        $label = $showAspirations
                                            ? app(\App\Services\Metaphysics\MingGuaCalculator::class)->getLifeAspiration($data['trigram_gua'] ?? $note->gua_number)
                                            : ($data['name'] ?? '???');
                                    @endphp
                                    <text x="{{ $sectorX + $sectorWidth * 0.5 }}" y="{{ $sectorY + $sectorHeight * 0.55 }}" fill="white"
                                        stroke="white" stroke-width="2" stroke-linejoin="round" text-anchor="middle" class="font-black"
                                        style="font-size: {{ max(12, ($bounds['x2'] - $bounds['x1']) / 25) }}px; opacity: 0.5;">{{ $label }}</text>
                                    <text x="{{ $sectorX + $sectorWidth * 0.5 }}" y="{{ $sectorY + $sectorHeight * 0.55 }}"
                                        fill="{{ $data['color'] ?? '#000' }}" text-anchor="middle" class="font-black"
                                        style="font-size: {{ max(12, ($bounds['x2'] - $bounds['x1']) / 25) }}px; opacity: 0.8;">{{ $label }}</text>
                                @endforeach
                                    ...
                                    @foreach($plan->baguaNotes as $note)
                                        ...
                                        <g class="flying-stars font-black"
                                            ...
                                            @if($note->mountain_star)
                                                <text x="{{ $sectorX + $sectorWidth * 0.15 }}" y="{{ $sectorY + $sectorHeight * 0.25 }}"
                                                    fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"
                                                    text-anchor="middle">{{ $note->mountain_star }}</text>
                                                <text x="{{ $sectorX + $sectorWidth * 0.15 }}" y="{{ $sectorY + $sectorHeight * 0.25 }}"
                                                    fill="#b45309" text-anchor="middle">{{ $note->mountain_star }}</text>
                                            @endif
                                            @if($note->water_star)
                                                <text x="{{ $sectorX + $sectorWidth * 0.85 }}" y="{{ $sectorY + $sectorHeight * 0.25 }}"
                                                    fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"
                                                    text-anchor="middle">{{ $note->water_star }}</text>
                                                <text x="{{ $sectorX + $sectorWidth * 0.85 }}" y="{{ $sectorY + $sectorHeight * 0.25 }}"
                                                    fill="#1d4ed8" text-anchor="middle">{{ $note->water_star }}</text>
                                            @endif
                                            @if($note->base_star)
                                                <text x="{{ $sectorX + $sectorWidth * 0.5 }}" y="{{ $sectorY + $sectorHeight * 0.9 }}"
                                                    fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"
                                                    text-anchor="middle">{{ $note->base_star }}</text>
                                                <text x="{{ $sectorX + $sectorWidth * 0.5 }}" y="{{ $sectorY + $sectorHeight * 0.9 }}"
                                                    fill="#52525b" text-anchor="middle">{{ $note->base_star }}</text>
                                            @endif
                                        </g>
                                    @endforeach
                                @endif

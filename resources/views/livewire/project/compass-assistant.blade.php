        ...
        <div class="space-y-6">
            ...
            <div x-show="step === 1" class="space-y-6">
                <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-6 border border-zinc-200 dark:border-zinc-700">
                    ...
                    <ul class="space-y-3 text-sm text-zinc-600 dark:text-zinc-300 list-disc list-inside">
                        ...
                        <li><strong>{{ __('Point the TOP of the phone INTO the room (away from the window).') }}</strong></li>
                ...
                <div class="flex justify-center py-4">
                    <div class="relative w-32 h-32 border-2 border-dashed border-zinc-300 rounded-lg flex items-center justify-center">
                        <span class="text-xs text-zinc-400 absolute top-2">{{ __('Window') }}</span>
                        <div class="flex flex-col items-center gap-1">
                            <flux:icon.device-phone-mobile class="size-12 text-zinc-800 dark:text-zinc-200 rotate-180" />
            ...
            <div x-show="step === 2" class="space-y-6">
                ...
                @if($floorPlans->count() > 1)
                    <flux:select wire:model="selectedFloorPlanId" label="{{ __('Select Floor Plan') }}">
                        @foreach($floorPlans as $fp)
                            <flux:select.option value="{{ $fp->id }}">{{ $fp->title ?? __('Floor Plan') }} {{ $loop->iteration }}</flux:select.option>
                        @endforeach
                    </flux:select>
                @endif
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        @if($selectedFloorPlanId && $floorPlans->find($selectedFloorPlanId))
                            <div class="relative w-full h-64 bg-zinc-100 dark:bg-zinc-800 rounded-lg overflow-hidden border border-zinc-300 cursor-crosshair image-container select-none touch-none"
                                 ...
                                 @touchmove.prevent.window="draw($event)"
                                 @touchend.window="stopDrawing()">
                                ...
                                <svg class="absolute inset-0 w-full h-full pointer-events-none">
                                    ...
                                    <!-- Visible arrow line -->
                                    <line x-show="isDragging || hasArrow"
                                          :x1="startX" :y1="startY" :x2="endX" :y2="endY"
                                    ...
                                    <!-- Start handle -->
                                    <circle x-show="hasArrow && !isDragging"
                                            :cx="startX" :cy="startY" r="6"
                                            fill="white" stroke="#f43f5e" stroke-width="2"
                                            class="pointer-events-auto cursor-grab" />

                                    <!-- End handle -->
                                    <circle x-show="hasArrow && !isDragging"
                                            :cx="endX" :cy="endY" r="6"
                                            fill="#f43f5e" stroke="white" stroke-width="2"
                                            class="pointer-events-auto cursor-grab" />

                                    <!-- Angle label on the arrow -->
                                    <template x-if="length > 25">
                                        <g>
                                            <rect :x="midX - 28" :y="midY - 12"
                                                  width="56" height="22" rx="4"
                                                  fill="rgba(255,255,255,0.92)" stroke="#f43f5e" stroke-width="1" />
                                            <text :x="midX" :y="midY + 3"
                                                  text-anchor="middle"
                                                  fill="#f43f5e" font-size="12" font-weight="700"
                                                  x-text="angleText"></text>
                                        </g>
                                    </template>
                                </svg>
                            </div>
                        ...
                        <div class="flex justify-between text-xs text-zinc-500">
                            <span>{{ __('Arrow Angle:') }} <span x-text="hasArrow ? angle.toFixed(1) + '°' : '-'"></span></span>
                            <span class="text-red-500" x-show="length > 0 && length < 10">{{ __('Line too short') }}</span>
                        </div>
                    ...
                    <div class="space-y-4">
                        <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
                             <h4 class="font-bold text-sm text-blue-800 dark:text-blue-300 mb-2">{{ __('Compass Reading') }}</h4>
                             <p class="text-xs text-blue-600 dark:text-blue-400 mb-4">{{ __('Enter the degrees shown on the phone when pointing INTO the room.') }}</p>

                             <flux:input wire:model="compassReading" type="number" step="0.1" min="0" max="360" label="{{ __('Reading (°)') }}" placeholder="0.00" />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-between pt-4">
                    ...
                    <flux:button variant="primary" wire:click="nextStep" x-bind:disabled="!compassReading || !hasArrow">{{ __('Calculate') }}</flux:button>
                </div>
            </div>
            ...
            <div x-show="step === 3" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-zinc-50 dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700 text-center">
                        <div class="text-xs text-zinc-500 uppercase tracking-wider mb-1">{{ __('North Deviation') }}</div>
                        <div class="text-2xl font-bold text-zinc-900 dark:text-white">{{ $calculatedNorthDeviation }}°</div>
                ...
                <div class="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800 text-sm text-green-800 dark:text-green-300">
